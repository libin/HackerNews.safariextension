/*
 *	Name:		Hacker News OnePage for Safari
 *  Version: 1.1.2
 *  Author:  Libin Pan
 *  http://blog.libinpan.com
 *  email: libinpan+HackerNews4Safari@gmail.com
 *
 *  It is created base on: 
 *  Hacker News OnePage for Chrome
 *	Version:	0.7.1 Beta
 *  Author:		Tim Dupree
 ...
 *				This is pretty much the same as the greasemonkey version, just unsafeWindow has been changed to 
 *				contentWindow and I leave the table based article header in rather than replacing it with the 
 *				CSS styled div based article header.  The div version deals with the resize better, but there
 *				was a problem appending it to the DOM, so I commented that line out.
 *
 */
	...
	*/
	...
	/* Add jQuery
	 * This "add jQuery" code is necessary for the geasemonkey version, 
	 * but we package jquery-1.3.2.min.js with the chrome extension instead.
	 */
  ...
  // function GM_wait() {
  //   if(typeof contentWindow.jQuery == 'undefined') { window.setTimeout(GM_wait,100); }
  // else { $ = contentWindow.jQuery; letsJQuery(); }
  // }
  // GM_wait();
	...
	function letsJQuery() {
		...
		if(hasForm.length == 1 && hasForm[0].action=="http://www.hnsearch.com/search#request/all"){
